...
- 如果一个方法有多个修饰器，那么该方法会先从外到内进入修饰器，然后由内向外执行,修饰器除了可以用注释，还可以用来进行类型检查，这对于类来说相当有用，他将是javascript代码静态分析的重要工具
- 修饰器不能用于函数，因为存在函数提升，只能用于类和类的方法，类不会提升，如果一定要修饰函数，可以使用高阶函数的形式直接执行
- core-decorators.js是一个第三方模块提供几个常见的修饰器
- 可以使用修饰器使得对象的方法被调用时自动发出一个事件,Postal.js是一个事件发布/订阅库
- 修饰器的基础上可以实现mixin模式，所谓mixin模式就是对象继承的一种代替方案，中文译为混入，意为在一个对象中混入另外一个对象的方法
- trail也是一种修饰器，效果与Mixin类似，但是提供了更多功能，比如防止同名方法冲突，排除混入某些方法，为混入的方法起别名等,这个模块提供的traits修饰器不仅可以接收对象，还可以接受es6类作为参数

## Module的语法

+ es6在语言规格层面实现了模块功能，并且实现的相当简洁，完全可以取代现有的CommobJS和AMD规范，成为浏览器和服务器通用的模块解决方案，es6模块的设计思想是尽量静态化，是的编译时就能确定模块的依赖关系，以及输入输出的变量，commitJS和AMD模块都只能在运行时确定这些东西，比如CommonJS模块就是对象，输入是必须查找对象属性
+ 由于es6模块化是编译时加载使得静态分析成为可能，有了他就能进一步拓展javascript的语法比如引入宏和类型检查这些只能靠静态分析实现的功能
+ es6模块自动采用严格模式，不管有没有在模块头部加上`use strict`
+ 模块功能主要有两个命令构成:export和import,export命令用于规定模块的对外接口，import命令用于输入其他模块提供的功能，使用export是接口名必须与模块内部变量建立一一对应关系，export语句输出的接口与其对应的值是动态绑定关系，即可以通过接口渠道米块内部实时的值
+ import具有提升效果，会提升到整个模块的头部并首先执行，import语句会执行所加载的模块，所以可以直接在import后面加上模块，多次导入只会执行一次，最好不要吧import和require写一个模块里，尽管有babel，模块整体加载所在的对象应该是可以静态解析的，所以不允许运行时改变
+ 使用default导出，不需要加大括号，import导入任意指定名称，也不需要加大括号，default在一个模块中只能使用一次
+ 如果一个模块之中先输入后输出同一个模块，import语句可以和export语句写在一起
+ 模块之间可以继承
+ 跨模块常量：当常量非常多时可以在一个文件夹的一个文件中合并，然后单独导出这一个文件
+ 由于import无法在运行时加载模块，所以在取代require是就成了一个障碍，import无法取代require的动态加载功能，所以有一个提案引入import()函数来完成动态加载，import能接受什么参数，import()就能接受什么参数，区别主要就是后者为动态加载，该函数可以用在任何地方，所加载的模块没有静态连接关系，这也与import不同，import()类似于node的require(),区别是前者是异步加载，后者是同步加载，使用场景：按需加载，条件加载，动态的模块路径
+ 注意点：import()加载模块成功后，这个模块会作为一个对象当做then方法的参数，因此可以用对象的解构赋值语法获取输出接口，如果有default也可以使用具名输入的形式，如果想同时加载多个模块可以作为一个数组放在Promise.all里面，import()也可用在async函数之中

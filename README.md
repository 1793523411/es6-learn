...
- 浏览器加载script，默认同步加载，有两种异步加载方法，defer或async，前者页面渲染完加载，后者下载完就执行
- 加载es6模块时要声明type=“moudle”属性，并且此模块默认异步加载，不会造成浏览器堵塞，等于打开了defer属性，当然后也可以手动打开async异步加载方式，es6模块也可以在内嵌在网页中，与外部加载脚本完全一致，利用顶层this等于undefined可以用来测试当前代码是否在es6模块之中
- es6与commonjs的差异，前者的模块输出是值得复制，后者输出是指的引用，前者是运行时加载，后者是编译时输出接口，前者加载的是一个对象，该对象只有在脚本运行结束时才会生成，后者不是对象，它的对外接口只是一种定义，在代码解析阶段就会生成
- 值的复制：模块内部的变化就影响不到这个值，值的引用：es6输入模块变量相当于unix中的符号连接，多个模块加载该模块加载的是同一个实例
- node加载，由于他与node自带的Commonjs不兼容，所以两者要分开，各自采用各自的加载方案，在静态解析阶段，只要有一行import或export，node就会认为这是es6模块，否则就是commonjs模块，如果不输出任何接口，但希望被认为是es6模块，可以在脚本上加入`export{}`，这不是输出一个空对象，而是不输出任何接口的es6的标准写法
- es6中顶层this指向undefined，commonjs模块的顶层this指向当前模块，两者查找文件的策略也略有所不同
- **在node中使用import命令加载CommonJS模块，node会自动将module.exports属性当做模块的默认输出环境**，即等于export default,如果采用整体输入的写法，default会取代module.exports作为输入的接口，通过bar。defaut这样的写法才能获取module.exports，因为bar本身不是一个对象，不能当做函数调用，只能通过bar.default调用,CommonJS模块的输出缓存机制在es6加载方式下依然有效
- import加载Commonjs模块必须整体输入，不允许只引入某个Commonjs模块的某一部分，因为Commonjs格式的模块在运行时才能确定具体的接口，而import要求编译时就确定这个接口
- **采用require加载es6模块时，es6模块的所有输入接口都会成为输入对象的属性**
- 循环加载：a脚本的执行依赖b脚本，而b脚本的执行依赖a脚本，commonjs模块遇到循环加载是返回的是当前已经执行的部分的值，而不是代码全部执行后的值，两者可能会有差异，所以输入变量时必须非常小心，所以引入变量时应该全部引入，而不要部分引入，不然会发生值被改写

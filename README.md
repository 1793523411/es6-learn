...
- 只要某种数据结构具有 Iterator 接口，都可以解构赋值
...
+ 原生具备Iterator接口的数据结构：Array，Map，Set，String，TypedArray，NodeList对象,其他数据结构（主要是对象）的Itrator接口都需要自己在Symbol。iterator属性上面部署，这样才会被for···of遍历
